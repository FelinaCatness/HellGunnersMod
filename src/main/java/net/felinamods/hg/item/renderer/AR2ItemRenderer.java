...
import net.felinamods.hg.item.model.AR2ItemModel;
import net.felinamods.hg.item.AR2Item;
import net.felinamods.hg.item.renderer.layer.AR2Layer;
...
public class AR2ItemRenderer extends GeoItemRenderer<AR2Item> {
   ...
   private float adsProgress = 0.0F;
	...
	public AR2ItemRenderer() {
	    ...
	    this.addRenderLayer(new AR2Layer(this));		
	}
	...
	@Override
public void actuallyRender(PoseStack poseStack, AR2Item animatable,
    ...
    if (!Minecraft.getInstance().options.bobView().get()) {
        ...
        if (player != null && this.transformType.firstPerson()) {
            ...
            if (stack.getItem() instanceof AR2Item) {
...
if (Math.abs(target - adsProgress) < 0.001f) {
   ...
   private float lerp(float t, float a, float b) {
	...
	@Override
	public void renderRecursively(PoseStack stack, AR2Item animatable, GeoBone bone, RenderType type, MultiBufferSource buffer, VertexConsumer bufferIn, boolean isReRender, float partialTick, int packedLightIn, int packedOverlayIn, int color) {
