...
import net.felinamods.hg.item.model.AR23lItemModel;
import net.felinamods.hg.item.AR23lItem;
import net.felinamods.hg.item.renderer.layer.AR23lLayer;
...
public class AR23lItemRenderer extends GeoItemRenderer<AR23lItem> {

   private float adsProgress = 0.0F;

	public AR23lItemRenderer() {
		super(new AR23lItemModel());
	    this.addRenderLayer(new AR23lLayer(this));		
	}

	@Override
	public RenderType getRenderType(AR23lItem animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
	...
	protected AR23lItem animatable;
	...
	@Override
public void actuallyRender(PoseStack poseStack, AR23lItem animatable,
    ...
    if (!Minecraft.getInstance().options.bobView().get()) {
        ...
        if (player != null && this.transformType.firstPerson()) {
            ...
            if (stack.getItem() instanceof AR23lItem) {
...
if (Math.abs(target - adsProgress) < 0.001f) {
   ...
   private float lerp(float t, float a, float b) {
	...
	@Override
	public void renderRecursively(PoseStack stack, AR23lItem animatable, GeoBone bone, RenderType type, MultiBufferSource buffer, VertexConsumer bufferIn, boolean isReRender, float partialTick, int packedLightIn, int packedOverlayIn, int color) {
	...
	@Override
	public ResourceLocation getTextureLocation(AR23lItem instance) {
