...
import net.felinamods.hg.item.model.MG43ItemModel;
import net.felinamods.hg.item.MG43Item;
import net.felinamods.hg.item.renderer.layer.MG43Layer;
...
public class MG43ItemRenderer extends GeoItemRenderer<MG43Item> {

   private float adsProgress = 0.0F;

	public MG43ItemRenderer() {
		super(new MG43ItemModel());
	    this.addRenderLayer(new MG43Layer(this));			
	}

	@Override
	public RenderType getRenderType(MG43Item animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
	...
	@Override
public void actuallyRender(PoseStack poseStack, MG43Item animatable,
    ...
    if (!Minecraft.getInstance().options.bobView().get()) {
        ...
        if (player != null && this.transformType.firstPerson()) {
            ...
            if (stack.getItem() instanceof MG43Item) {
...
if (Math.abs(target - adsProgress) < 0.001f) {
   ...
   private float lerp(float t, float a, float b) {
	...
	@Override
	public void renderRecursively(PoseStack stack, MG43Item animatable, GeoBone bone, RenderType type, MultiBufferSource buffer, VertexConsumer bufferIn, boolean isReRender, float partialTick, int packedLightIn, int packedOverlayIn, int color) {
	...
	@Override
	public ResourceLocation getTextureLocation(MG43Item instance) {
