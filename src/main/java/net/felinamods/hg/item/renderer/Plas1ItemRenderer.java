...
import net.felinamods.hg.item.model.Plas1ItemModel;
import net.felinamods.hg.item.Plas1Item;
import net.felinamods.hg.item.renderer.layer.Plas1Layer;
...
public class Plas1ItemRenderer extends GeoItemRenderer<Plas1Item> {

   private float adsProgress = 0.0F;

	public Plas1ItemRenderer() {
		super(new Plas1ItemModel());
	    this.addRenderLayer(new Plas1Layer(this));			
	}
	...
	@Override
public void actuallyRender(PoseStack poseStack, Plas1Item animatable,
    ...
    if (!Minecraft.getInstance().options.bobView().get()) {
        ...
        if (player != null && this.transformType.firstPerson()) {
            ...
            if (stack.getItem() instanceof Plas1Item) {
...
if (Math.abs(target - adsProgress) < 0.001f) {
   ...
   private float lerp(float t, float a, float b) {
	...
	@Override
	public void renderRecursively(PoseStack stack, Plas1Item animatable, GeoBone bone, RenderType type, MultiBufferSource buffer, VertexConsumer bufferIn, boolean isReRender, float partialTick, int packedLightIn, int packedOverlayIn, int color) {
