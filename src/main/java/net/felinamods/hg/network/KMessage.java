...
import net.felinamods.hg.procedures.MoveBackwardProcedure;
import net.felinamods.hg.HgMod;

@EventBusSubscriber(bus = EventBusSubscriber.Bus.MOD)
public record KMessage(int eventType, int pressedms) implements CustomPacketPayload {
	public static final Type<KMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(HgMod.MODID, "key_k"));
	...
	public static void pressAction(Player entity, int type, int pressedms) {
		...
		if (type == 1) {
			...
			MoveBackwardProcedure.execute(entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		HgMod.addNetworkMessage(KMessage.TYPE, KMessage.STREAM_CODEC, KMessage::handleData);
	}
}
